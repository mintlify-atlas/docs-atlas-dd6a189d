---
title: "User Roles and Access Control in VinylVibes"
...
When `esDemo` is `true` and `esAdmin` is `false`, the admin panel enters read-only mode. The following restrictions are applied at render time:
